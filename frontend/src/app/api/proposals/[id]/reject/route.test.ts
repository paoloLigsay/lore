...
function callRoute() {
  const request = new NextRequest(`http://localhost/api/proposals/${PROPOSAL_ID}/reject`, {
    method: "POST",
  });
...
describe("POST /api/proposals/[id]/reject", () => {
  ...
  it("marks the proposal REJECTED without touching the note", async () => {
    mockAuthedUser(USER_ID);
    const proposal = baseProposal();
    mockPrisma.proposal.findUnique.mockResolvedValue(proposal);
    mockPrisma.proposal.update.mockResolvedValue({ ...proposal, status: "REJECTED" });
